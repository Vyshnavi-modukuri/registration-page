function user() {
    ...
    else if(name[0]>='A' && name[0]<='Z')
    {
        ...
        {
            if (specialCharacters.indexOf(name[i]) !== -1) 
            {
                error1.innerHTML="";
                flag=1;
            }
            ...
            error1.innerHTML="Username should contain atleast one Special Character";
        }
    }
    else
        error1.innerHTML = "First character should be captial";}
...
function repassword(){
    let re_password=document.getElementById('b').value;
    if(document.getElementById('d').value != re_password){
        error4.innerHTML = "*Password doesn't matches";
        return false;
    }
    else{
